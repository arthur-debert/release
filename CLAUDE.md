...
- `bin/` — local CLI tools that mutate consumer-repo state or drive the day-to-day PR loop. **Single source of truth for everything on `$PATH`.** Includes:
  - Policy/setup (canonical `release-core admin policy|secrets …`; flat aliases in parens): ruleset (`apply-ruleset`), sweep (`sweep-github-policy`), dependabot (`enable-dependabot-security`); `release-core admin secrets install|token` (`install-release-{secrets,token}`); plus `detect-kind`
