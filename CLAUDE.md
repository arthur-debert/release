...
- **The CLI is the interface:** all infra/fleet tasks go through the
  ...
  (#521) — `.release/` is now gitignored + installed by `release-core init`, so
...
- `bin/` — **not a consumer surface** (the wheel + bootstrap quartet carry everything consumers need). It holds only what genuinely can't be a `release-core` subcommand: the in-checkout dev entry (`bin/release-core`), pre-boot scripts that run before `release_core` exists (`install-release-core`), standalone stdlib fetchers pulled raw over HTTP (`fetch-deps`/`fetch-artifact`), and fleet-operator tools (`orc`, `clone-lex-*`). On release's own `$PATH` via dodot. **The maintainer surface listed below is the `release-core` CLI, NOT `bin/` scripts** — mapped here so you know where each capability lives:
  ...
  - Install the `.release/` temp dir: `release-core init` writes the gitignored, ephemeral `.release/` temp dir + the committed mirrors (symlinks, real-file workflow copies, CLAUDE.md block) from the pinned wheel. WS4 (#521) made `.release/` gitignored + rewritten every session/CI, so it can't fall out of sync — the standalone `release-sync` builder and the `release-drift-check` check (+ the `sync` CLI group) were RETIRED. The install steps live in `release_core/sync.py` (`install_plan`/`install_tree`/`compute_mirror` — `install_tree`/`install_plan` were the jargon-named `materialize`/`build_plan`, renamed under #667); `release-core init` is their only caller. The `.release/.release-sync-source` provenance marker (ADR-0002) is still written but transient + informational (no reader). CI installs `.release/` via the `arm-gate` composite's `install_tree` input (default on; `materialize` on the still-pinned @v2) before the gate.
