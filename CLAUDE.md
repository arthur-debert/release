...
- `bin/` — local CLI tools that mutate consumer-repo state or drive the day-to-day PR loop. **Single source of truth for everything on `$PATH`.** Includes:
  ...
  - PR loop: `release-core pr review request|cancel|wait|show [--reviewer <name>]` (reviewer-agnostic, dispatches through the adapter registry in `prstate/reviewers.py` — the RETIRED `pr copilot on|off|wait|review` group and the `gh-copilot-*` bin scripts are gone, no aliases, per #555), plus `gh-pr-checks-wait`, `gh-pr-resolve-thread` (the `gh-release-issue` consumer escalation tool is now a pip console-script, retired as a `bin/` shim in #476)
...
- **Every PR is driven through the `gh-pr-review-loop` skill — invoke it before
  `gh pr create`, not after.** The skill is the *discipline* (request the
  required reviews → wait → triage A/B/C → resolve threads as you go → stop at
  ready); the `release-core pr review …` / `gh-pr-checks-wait` helpers are each
