...
Usage:
  ...
  audit-portfolio --markdown                   # render the fleet matrix (docs/fleet-matrix.md)
...
def main(argv: list[str]) -> int:  # noqa: C901 — flat dispatch mirrors the bash modes
    ...
    if markdown_mode:
        print(_render_markdown(summary_rows), end="")
        return 0
...
def _check_message(results: list[tuple[str, str, str]], name: str) -> tuple[str, str]:
    """(status, message) for the named check, or ('', '') if absent."""
    for status, n, msg in results:
        if n == name:
            return status, msg
    return "", ""


def _shared_cell(results: list[tuple[str, str, str]]) -> str:
    """The shared-vs-OWN workflow cell from the workflows_thin_callers check."""
    status, msg = _check_message(results, "workflows_thin_callers")
    if status == "PASS":
        return "OK (all thin callers)"
    if status == "WARN" and "bespoke:" in msg:
        return "OWN: " + msg.split("bespoke:", 1)[1].strip()
    if status == "SKIP":
        return "—"
    return msg or "—"


def _findings_cell(results: list[tuple[str, str, str]]) -> str:
    """Comma-joined names of failing/warning checks, or '—' when clean."""
    items = [name for status, name, _ in results if status in ("FAIL", "WARN")]
    return ", ".join(items) if items else "—"


def _render_markdown(summary_rows: list[tuple]) -> str:
    """The fleet matrix as Markdown — the generated docs/fleet-matrix.md body."""
    ...
    lines = [
        ...
        "",
        f"> GENERATED by `release-core admin repos audit --markdown` on {stamp}.",
    ...
    total_pass = 0
    total_app = 0
    for ec, fails, warns, passes, _skips, _conf, repo, results in summary_rows:
        total_pass += passes
        total_app += passes + fails + warns
        status = _STATUS_BY_EC.get(ec, f"?({ec})")
        lines.append(f"| {repo} | {status} | {_shared_cell(results)} | {_findings_cell(results)} |")
    portfolio = (100 * total_pass // total_app) if total_app > 0 else 0
    lines += [
        "",
        f"**Portfolio conformance:** {portfolio}% "
        f"({total_pass}/{total_app} applicable checks pass).",
        "",
        "`OWN` = the repo carries a bespoke workflow not shared from "
        "`arthur-debert/release`; `—` = not applicable. The `release_token`, "
        "`ruleset`, and `dep_security` checks SKIP when the run's token can't "
        "read repo-admin data (e.g. a read-only fleet token), so they don't "
        "show as false failures here.",
        "",
    ]
    return "\n".join(lines)
