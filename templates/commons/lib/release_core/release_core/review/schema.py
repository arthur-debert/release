...
(`--output-schema`) and the agy backend describes in-prose. It is the phos
schema with the dual-repo `repository` enum dropped: a single-repo review has
no need to attribute a comment to one of two repositories.

`extract_json` ports the phos scripts' three-fallback parse: direct, fenced
(```json …```), then a greedy `{...}` regex — agents wrap their JSON output
inconsistently, so the parser is deliberately forgiving.
"""
