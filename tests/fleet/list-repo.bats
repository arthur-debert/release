...
@test "list-repo-pr --help exits 0 and prints usage" {
  run "$BIN/release-core" admin repos prs --help
  [ "$status" -eq 0 ]
  [[ "$output" == *"Usage:"* ]]
  [[ "$output" == *"list-repo-pr"* ]]
}
...
@test "list-repo-pr unknown flag exits 64" {
  run "$BIN/release-core" admin repos prs --bogus
  [ "$status" -eq 64 ]
  [[ "$output" == *"unknown arg"* ]]
}

@test "list-repo-scripts --help exits 0 and prints usage" {
  run "$BIN/release-core" admin repos scripts --help
  [ "$status" -eq 0 ]
  [[ "$output" == *"Usage:"* ]]
...
@test "list-repo-scripts unknown flag exits 64" {
  run "$BIN/release-core" admin repos scripts --bogus
  [ "$status" -eq 64 ]
  [[ "$output" == *"unknown arg"* ]]
}
...
@test "list-repo-scripts --owner with no present repos still prints totals footer" {
  ...
  run "$BIN/release-core" admin repos scripts --owner nonesuch-owner
  [ "$status" -eq 0 ]
