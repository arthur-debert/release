...
| `release-issue-relay` synced skill | **drop from distribution** | — | WS2's exit allows "at most one thin delegating skill"; this was the second. The contract + mechanism are binary-carried (above); a skill file added discovery surface without adding capability. Consumers' copies are swept by the broken-symlink cleanup on their next init. |
...
| `gh-pr-review-loop` synced skill | **keep, ephemeral** | wheel → mirror, untracked | The one thin delegating skill (the `/`-trigger surface the harness needs as a file). Materialized by init, never tracked. |
...
materialized by every `init` (exactly as before) but never tracked:
