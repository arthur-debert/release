...
#      present but release-core is on PATH, build the managed
...
# LEFTHOOK_CONFIG. .release/lefthook.yml exists only when the caller built
# it (e.g. rust-cli's arm-gate before prepare-release); when it is absent this is a
# no-op and detection falls through to the release-core branch, which builds
...
if [ -n "${LEFTHOOK_CONFIG:-}" ]; then
  ...
  # outside rust-cli's arm-gate nothing built .release/ — so this branch
  ...
  # install-release-core), so: build the managed tree from the wheel
  ...
  echo "No root gate config but release-core is present — building the managed gate."
